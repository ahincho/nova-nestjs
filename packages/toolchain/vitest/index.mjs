 ...
 * es el directorio del archivo de configuración del proyecto. A diferencia del
 * preset de Jest que reemplaza, acá no hace falta esquivar `<rootDir>`.
 *
 * No declara nada sobre la transformacion de TypeScript a propósito. Oxc, que
 ...
 * Declararlo acá además no sirve: `tsconfigRaw` no le gana al tsconfig del
 * proyecto, comprobado poniendolo en false y viendo que la metadata se seguia
 * emitiendo.
 *
 * El umbral de cobertura vive acá a propósito: si cada repo elige el suyo, el
 * número deja de significar algo al compararlos.
 */
/**
 ...
 * módulos, y desde NestJS 12 ese grafo es ESM y pesa mas: medido en 740 ms con
 * la máquina libre, y visto pasar de 5 s con el build, el typecheck y el lint
